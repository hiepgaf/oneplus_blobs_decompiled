  ...
  extends Filter
{
  ...
  private void createHistogramFrame(FilterContext paramFilterContext, int paramInt1, int paramInt2, int[] paramArrayOfInt)
  {
    ...
    int k = (int)(paramInt2 * 0.05F);
    int m = (int)(paramInt1 * 0.05F);
    int i = k;
    while (i < paramInt2 - k)
    {
      int j = m;
    ...
    while (i < 766)
    {
      arrayOfInt[i] = ((int)(arrayOfInt[i] * 65535L / ((paramInt1 - m * 2) * (paramInt2 - k * 2))));
      i += 1;
    }
    ...
    this.mHistFrame = paramFilterContext.getFrameManager().newFrame(paramArrayOfInt);
    this.mHistFrame.setInts(arrayOfInt);
  }
  ...
  public void initProgram(FilterContext paramFilterContext, int paramInt)
  {
    ...
    this.mShaderProgram = paramFilterContext;
  ...
  protected void prepare(FilterContext paramFilterContext)
  {
    ...
    while (i < 1024)
    {
      arrayOfInt[i] = ((int)(normal_cdf[i] * 65535L / 766L));
      i += 1;
    }
    MutableFrameFormat localMutableFrameFormat = ImageFormat.create(1024, 1, 3, 3);
    this.mDensityFrame = paramFilterContext.getFrameManager().newFrame(localMutableFrameFormat);
    this.mDensityFrame.setInts(arrayOfInt);
  }
  ...
  public void process(FilterContext paramFilterContext)
  {
    ...
    if ((this.mShaderProgram == null) || (((FrameFormat)localObject).getTarget() != this.mTarget))
    {
    ...
    if ((((FrameFormat)localObject).getWidth() != this.mWidth) || (((FrameFormat)localObject).getHeight() != this.mHeight))
    {
      ...
      createHistogramFrame(paramFilterContext, this.mWidth, this.mHeight, localFrame1.getInts());
    }
    paramFilterContext = paramFilterContext.getFrameManager().newFrame((FrameFormat)localObject);
    localObject = this.mHistFrame;
    Frame localFrame2 = this.mDensityFrame;
    this.mShaderProgram.process(new Frame[] { localFrame1, localObject, localFrame2 }, paramFilterContext);
...
/* Location:              /Users/joshua/Desktop/system_framework/classes-dex2jar.jar!/android/filterpacks/imageproc/AutoFixFilter.class
