...
public class NotificationManagerCompat
{
  ...
  public static Set<String> getEnabledListenerPackages(Context paramContext)
  {
    ...
    for (;;)
    {
      ...
      if (!paramContext.equals(sEnabledNotificationListeners))
      {
        ??? = paramContext.split(":");
  ...
  private static boolean useSideChannelForNotification(Notification paramNotification)
  {
    ...
    while (!paramNotification.getBoolean("android.support.useSideChannel")) {
      return false;
    }
    return true;
  }
    ...
    implements Handler.Callback, ServiceConnection
  {
    ...
    private void processListenerQueue(ListenerRecord paramListenerRecord)
    {
      if (!Log.isLoggable("NotifManCompat", 3))
      {
        ...
        if (ensureServiceBound(paramListenerRecord)) {
          break label85;
        }
      }
      label84:
      label85:
      while (paramListenerRecord.service == null)
      {
      ...
      for (;;)
      {
        ...
        {
          while (!Log.isLoggable("NotifManCompat", 3))
          {
            if (!paramListenerRecord.taskQueue.isEmpty()) {
              break label250;
            }
            return;
            Log.d("NotifManCompat", "Sending task " + localDeadObjectException);
          }
        }
    ...
    private void scheduleListenerRetry(ListenerRecord paramListenerRecord)
    {
      ...
      if (!this.mHandler.hasMessages(3, paramListenerRecord.componentName))
      {
        ...
        i = (1 << paramListenerRecord.retryCount - 1) * 1000;
        if (Log.isLoggable("NotifManCompat", 3)) {
          break label154;
        }
      }
    ...
    private void updateListenerMap()
    {
      ...
      } while (localHashSet.contains(((Map.Entry)localObject2).getKey()));
      if (!Log.isLoggable("NotifManCompat", 3)) {}
      for (;;)
      {
...
/* Location:              /Users/joshua/Desktop/system_framework/classes-dex2jar.jar!/android/support/v4/app/NotificationManagerCompat.class
